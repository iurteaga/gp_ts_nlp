...
# If EVALUATE is not provided, just assume 1
if [ -z "$EVALUATE" ]
then
    EVALUATE='false'
fi
...
# Start
echo "Starting Random Fairseq RoBERTa script for $MAX_INTERACTIONS interactions at $(date)"
...
do
    ...
    echo "Starting random interaction $INTERACTION/$MAX_INTERACTIONS up to $MAX_UPDATE_COUNT updates at $(date)"
    ...
    fairseq-hydra-train -m --config-dir $OUTPUT_DIR \
        ...
        task.data=$PATH_TO_PRETRAIN_DATA \
    ...
    ### Random code
    # Use random pull to update fairseq config
    python3 ./nlp_bandit_scripts/random_update_fairseq_hydra.py \
    ...
    echo "Finished random interaction $INTERACTION/$MAX_INTERACTIONS at $(date)"
...
# Finished
echo "Done Random Fairseq RoBERTa script for $MAX_INTERACTIONS interactions at $(date)"
