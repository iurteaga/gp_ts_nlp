...
{
    echo ""
    echo "Usage: $0 -p PATH_TO_PRETRAIN_CONFIG -d PATH_TO_PRETRAIN_DATA -f PATH_TO_FINETUNE_CONFIG_DIR -e EVALUATE -s SEED -m MACHINE"
...
while getopts i:u:p:d:f:e:s:m:l: arg
do
    case "$arg" in
...
if [ -z "$PATH_TO_PRETRAIN_CONFIG" ] || [ -z "$PATH_TO_PRETRAIN_DATA" ]
then
...
# Output dir is based on pretrain and finetune configs (plus seed)
# Pretrain
...
then
    ...
    EXPERIMENT_NAME="$PRETRAIN_CONFIG"_"$FINETUNE_CONFIG_NAME"
else
    EXPERIMENT_NAME="$PRETRAIN_CONFIG"
fi
...
# Fairseq config
PATH_TO_CLASSIC_PRETRAIN_CONFIG="$OUTPUT_DIR/$PRETRAIN_CONFIG_FILENAME"
...
cp $PATH_TO_PRETRAIN_CONFIG $PATH_TO_CLASSIC_PRETRAIN_CONFIG
...
then
    ...
    do
        ...
        PATH_TO_CLASSIC_FINETUNE_CONFIG="$FINETUNE_OUTPUT_DIR/$FINETUNE_CONFIG_FILENAME"
        # Finetune specific output dir 
        mkdir -p $FINETUNE_OUTPUT_DIR
        
        # Replicate initial config within fine-tuning output dir
        cp $PATH_TO_FINETUNE_CONFIG_FILE $PATH_TO_CLASSIC_FINETUNE_CONFIG
    done
fi

#################### START SCRIPT ########################
# Executed script info
echo "Executing script $0 at $(date) with"
...
# Start
echo "Starting Classic Fairseq RoBERTa script for $MAX_INTERACTIONS interactions at $(date)"
...
do
    ...
    echo "Starting Classic Fairseq RoBERTa interaction $INTERACTION/$MAX_INTERACTIONS up to $MAX_UPDATE_COUNT updates at $(date)"
    ...
    echo "Finished Classic Fairseq RoBERTa interaction $INTERACTION/$MAX_INTERACTIONS at $(date)"
done

# Finished
echo "Done Classic Fairseq RoBERTa script at $(date)"
